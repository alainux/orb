...
test("voice has orchestration-only tooling and never configures or mirrors Pi",async()=>{
  const files=await Promise.all(["extensions/voice.ts","src/controller.ts","src/providers/gemini.ts","src/providers/openai.ts"].map(path=>readFile(path,"utf8")));
  const joined=files.join("\n");
  assert.match(joined,/run_pi_task/);assert.match(joined,/read_pi_log/);assert.match(joined,/observe_pi/);assert.doesNotMatch(joined,/control_pi/);assert.match(joined,/scratchpad/);
  assert.doesNotMatch(joined,/permissions\.cancelPi/);

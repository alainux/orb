...
- `src/orb.ts` — deterministic themed orb with one visual language: a full sphere of dots rendered with the lab's negative-space technique (`site/orb-braille-source-math-variations.html`), with listening's traveling wave pattern as the base carve for every state. The sphere is lit by a real two-light model: a slowly drifting key light (so shadows, the diffuse terminator, and a Phong highlight travel over the surface without rotating the point cloud), a fixed fill light, fresnel rim lighting along the silhouette, and a coarse object-space material grain — the orb reads as a lit solid, not a flat circle. Microphone/voice energy widens the carved grooves and, once loud enough, physically breaks the surface apart: edge points explode outward along their normal and scatter tangentially, and the silhouette fragments into bright fringe particles beyond the circle on audio peaks. Muted or silent frames get none of that disturbance — the base wave keeps traveling at its minimum under the same light (muted renders a slightly smaller sphere plus gray color, so it still reads as off). The three states differ only in color identity (see `src/widget.ts`): `smoke` (listening) is the calm themed gradient that intensifies with the mic, `composing` (talking) the bright audio envelope with white-hot transient flares, and `searching` (working) a calmer, cooler look. The sphere is deliberately never rotated as a rigid body (rotating point clouds alias into shimmer at terminal resolution) — the wave travels by the animation clock phase instead, and the clock never stops (the wave keeps flowing even while muted). The shading floor sits just above the braille dot threshold (`FIELD_FLOOR = 0.26` vs `0.24`), so even the terminator keeps its dim dots and the carved grooves read as gaps across the whole sphere, sweeping bottom-right → upper-left instead of dying out on the dark side. Mode switches dissolve between renderings over ~0.55s instead of popping. Audio reactivity scales with `ui.orbReactivity`. With `ui.orbBraille` enabled, every mode is rasterized into a 2×4 subpixel grid and packed into 8-dot Braille glyphs (U+2800+mask) for a much denser, finer-grained orb — the same theme/audio color pipeline applies per cell.

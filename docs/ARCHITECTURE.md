...
- `src/orb.ts` — deterministic themed orb with one visual language: a full sphere of dots rendered with the lab's negative-space technique (`site/orb-braille-source-math-variations.html`), with listening's traveling wave pattern as the base carve for every state. Microphone/voice energy widens the carved grooves and adds disturbance; muted or silent frames keep the same base wave traveling at its minimum disturbance (muted renders a slightly smaller sphere plus gray color, so it still reads as off). The three states differ only in color identity (see `src/widget.ts`): `smoke` (listening) is the calm themed gradient that intensifies with the mic, `composing` (talking) the bright audio envelope with white-hot transient flares, and `searching` (working) a calmer, cooler look. The sphere is deliberately never rotated as a rigid body (rotating point clouds alias into shimmer at terminal resolution) — the wave travels by the animation clock phase instead, and the clock never stops (the wave keeps flowing even while muted). Mode switches dissolve between renderings over ~0.55s instead of popping. Audio reactivity scales with `ui.orbReactivity`. With `ui.orbBraille` enabled, every mode is rasterized into a 2×4 subpixel grid and packed into 8-dot Braille glyphs (U+2800+mask) for a much denser, finer-grained orb — the same theme/audio color pipeline applies per cell.

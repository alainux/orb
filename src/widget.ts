...
interface WidgetOptions { orbAspect:number; orbDensity:number; panelHeight:number; activityLines:number }
...
export class VoiceWidget implements Component {
  ...
  private renderFrame(width:number):string[] {
    ...
    const height=Math.max(9,Math.min(24,this.options.panelHeight));
    ...
    const logLines=this.renderActivity(state.activity,rightWidth,Math.min(bodyHeight,this.options.activityLines));
    ...
    const body=Math.max(left.length,logLines.length);
    for(let i=0;i<body;i++)lines.push(`${padVisible(left[i]??"",leftWidth)}${" ".repeat(gap)}${logLines[i]??""}`);
    const meters=`YOU ${bar(this.frame.userEnergy,9)}  VOICE ${bar(this.frame.agentEnergy,9)}  ${this.frame.source.toUpperCase()}  audio ${state.audioQueuedMs}ms · drops ${state.audioCaptureDrops}`;
  ...
  private renderCompact(width:number,state:VoiceViewState):string[] {
    const h=Math.max(5,Math.min(7,this.options.panelHeight-3));

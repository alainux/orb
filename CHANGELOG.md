...
- Removed the `/voice settings` panel and its session-persistence machinery (`VOICE_PREFS_ENTRY`, `appendEntry` + branch-restore, `src/settings.ts`, the pi-tui `SettingsList`); toggling displays no longer writes session metadata. Removed the `.orb/config.json` override so the package ships on pure defaults.
